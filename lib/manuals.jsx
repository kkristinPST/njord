...
const SCADA_MANUAL = {
  ...
  sections: [
    ...
    { id: "users", title: "User administration", blocks: [
      ...
      { t: "ul", x: [
        "Users: create, edit and deactivate accounts, set contact details and the channels an account can receive on, and assign roles. Deactivating is the normal end of an account: the person stops being paged and loses access, but their name stays readable in every log that already records it. Deleting is for accounts created in error.",
        ...
        "On-call: alarm groups, the duty roster and shifts. See the next section.",
        "Preferences: theme (Light, Dark or Legacy), table density, default screen and units. Stored per device from the user menu.",
      ]},
      { t: "note", x: "The Users table shows what each account is on call for, and flags anyone who is the only first-line recipient of a group. Deactivating or removing such an account warns that the group would then page nobody." },
    ]},
    { id: "oncall", title: "On-call and the duty roster", blocks: [
      { t: "p", x: "On-call answers two different questions, and Settings keeps them apart. Who does an alarm go to, in what order, on which channel? And who is reachable tonight? The first is configuration and changes rarely; the second changes at every handover. Three views over one model: Groups, Duty roster and By person." },
      { t: "sub", x: "Groups" },
      { t: "p", x: "An alarm group carries up to three escalation tiers. Each tier lists its recipients (people or duty phones) and the channels it dispatches on, and the group carries a paging window and, optionally, the areas it covers. A resend interval repeats the notification until the alarm is acknowledged or returns to normal. NS 9416 requires two independent remote paths; SMS and voice count as one, because they share the mobile network." },
      { t: "sub", x: "Shifts and the duty roster" },
      { t: "p", x: "A shift is a named duty window, not a person's working hours: Daytime, Duty phone, Backup, Around the clock. A day can hold more than one interval, which is how a break inside a working day is expressed, and an interval that ends before it starts runs past midnight. The duty roster assigns recipients to shifts, with Off duty as a visible column rather than an absence." },
      { t: "p", x: "An alarm reaches someone when the group's paging window and that person's shift both allow it." },
      { t: "p", x: "The roster can be planned ahead. The usual week repeats until someone changes it; selecting a date shows the roster as it will be then, and any change is stored as an exception to the usual week for that day alone. A date carrying its own changes is marked in the day strip, and a single action resets it." },
      { t: "sub", x: "Coverage" },
      { t: "p", x: "Coverage checks the roster against the groups and reports what it finds, worst first: a group with nobody on Priority 1, a group whose whole first line is off duty, hours inside a paging window that no rostered shift covers, recipients who cannot receive any channel their tier dispatches on, groups with no third line, and people rostered but paged by nothing." },
      { t: "note", x: "Every change to groups, shifts and the roster is recorded with who made it. Open the change log from the history icon in the On-call toolbar." },
    ]},
    { id: "mobile", title: "Mobile view", blocks: [
      ...
      { t: "ul", x: [
        ...
        "On call: who is on duty now, what each group pages and in what order, and taking or handing over a shift for the day.",
      ...
      { t: "note", x: "Some things stay in the control room on purpose: process diagrams are not editable on a phone, the welfare bubble chart and timeline need a wide screen, reverting a change in an alarm's history is a control-room action, and on-call groups, tiers, channels and paging windows are configured at the desk. The phone changes today's roster, never the usual week." },
    ]},
  ],
};
