...
function D2OxCol({ cx, n, kar, hz, bar, open }) {
  ...
  return (
    <g>
      <Tag2 x={cx} y={612} tag={tag + "-PU1"} desc={["Oxygenation pump", "tank " + kar]} />
      <RD x={cx - 33} y={648} w={66} value={String(hz)} unit="Hz" tag={tag + "-PU1"} name={"Oxygenation pump tank " + kar + " speed"} group="Oxygenation" />
      <Eq title={"Oxygenation pump tank " + kar} onClick={d2Eq(tag + "-PU1", "Oxygenation pump tank " + kar, "pump", { primary: { l: "Speed", v: String(hz), u: "Hz" },
      ...
      <SymTrend cx={cx - 40} cy={704} tag={tag + "-PU1"} name={"Oxygenation pump tank " + kar} group="Oxygenation" running />
      <Eq title={"Dose valve tank " + kar} onClick={d2Eq(tag + "-SV1", "Dose valve tank " + kar, "valve", { primary: { l: "Opening", v: open ? "100" : "0", u: "%" }, readouts: [{ l: "Valve opening", v: open ? "100" : "0", u: "%", tag: tag + "-SV1" }] })}>
        <SymValve cx={cx} cy={856} running={open} />
      </Eq>
      <ModeChip x={cx + 20} y={848} mode="A" />
      <Eq title={"O₂ cone tank " + kar} onClick={d2Eq(tag + "-PT1", "O₂ cone tank " + kar, "vessel", { primary: { l: "Cone pressure", v: bar.toFixed(1), u: "bar" }, canStartStop: false, noMode: true,
      ...
      <RD x={cx - 33} y={1000} w={66} value={bar.toFixed(1)} unit="bar" tag={tag + "-PT1"} name={"Cone pressure tank " + kar} group="Oxygenation" />
      <Tag2 x={cx} y={1046} tag={tag + "-PT1"} desc={["Pressure in O₂ cone " + kar]} />
    </g>
  );
}
...
function Dpt2RasMimic() {
  ...
  const colStubs = D2_OX.map((c) => [
    `M${c.cx},672 V684`,        // header → pump top
    `M${c.cx},725 V840`,        // pump → dose valve
    `M${c.cx},872 V940`,        // valve → cone (top neck)
    `M${c.cx},972 V1012`,       // cone → collector
    `M${c.cx},814 V840`,        // O₂ header → valve
  ]).flat();
  return (
    <svg className="rasm" viewBox="0 0 2000 1100" role="img" aria-label="DPT2 RAS process mimic" preserveAspectRatio="xMidYMid meet">
      ...
      {[
        ...
        { k: "proc",  d: "M600,1012 H1866 V964" },
        // lye dosing loop → bioreactor
        { k: "chem",  d: "M60,818 H330" }, { k: "chem", d: "M330,838 H420 V500" }, { k: "chem", d: "M330,1000 H400 V838" },
      ].concat(colStubs.map((d) => ({ k: "proc", d }))).map((p, i) => <path key={"p" + i} d={p.d} className={"rasm-pipe fl-" + p.k} />)}

      {/* ───── FILTER / BACKWASH ───── */}
      <Flag x={40} y={438} label="Fish tanks" dir="r" />
      <Tag2 x={150} y={258} tag="DPT2-FIL0-PU1" desc={["Backwash pump filter 1"]} />
      <RD x={117} y={288} value="0" unit="Hz" tag="DPT2-FIL0-PU1" name="Backwash pump filter 1 speed" group="Filter" />
      <Eq title="Backwash pump filter 1" onClick={d2Eq("DPT2-FIL0-PU1", "Backwash pump filter 1", "pump", { primary: { l: "Speed", v: "0", u: "Hz" }, status: "low", readouts: [{ l: "Speed", v: "0", u: "Hz", tag: "DPT2-FIL0-PU1" }] })}><SymPump cx={150} cy={340} running={false} /></Eq>
      <ModeChip x={172} y={332} mode="A" />

      <Tag2 x={150} y={706} tag="DPT2-FIL0-PU2" desc={["Backwash pump filter 2"]} />
      <RD x={117} y={658} value="0" unit="Hz" tag="DPT2-FIL0-PU2" name="Backwash pump filter 2 speed" group="Filter" />
      <Eq title="Backwash pump filter 2" onClick={d2Eq("DPT2-FIL0-PU2", "Backwash pump filter 2", "pump", { primary: { l: "Speed", v: "0", u: "Hz" }, status: "low", readouts: [{ l: "Speed", v: "0", u: "Hz", tag: "DPT2-FIL0-PU2" }] })}><SymPump cx={150} cy={630} running={false} /></Eq>
      <ModeChip x={172} y={622} mode="A" />

      <Tag2 x={322} y={246} tag="DPT2-FIL1-FE1" desc={["Drum filter 1"]} />
      <RD x={289} y={270} value="0" unit="Hz" tag="DPT2-FIL1-FE1" name="Drum filter 1 speed" group="Filter" />
      <Eq title="Drum filter 1" onClick={open("DPT2-FIL0")}>
      ...
      <Tag2 x={322} y={536} tag="DPT2-FIL2-FE1" desc={["Drum filter 2"]} />
      <RD x={289} y={560} value="0" unit="Hz" tag="DPT2-FIL2-FE1" name="Drum filter 2 speed" group="Filter" />
      <Eq title="Drum filter 2" onClick={open("DPT2-FIL0")}>
      ...
      <Tag2 x={243} y={498} tag="DPT2-FIL0-LT1" desc={["Level before filter"]} />

      {/* ───── LYE DOSING ───── */}
      <Flag x={40} y={801} label="Lye" dir="r" />
      ...
      <Eq title="Blower 1 MBBR" onClick={open("DPT2-AEB0-BL1")}>
      ...
      <Tag2 x={988} y={330} tag="DPT2-AEB0-BM1" desc={["Blower 1 MBBR"]} />

      <Tag2 x={1153} y={150} tag="DPT2-AEB0-BM2-TT1" desc={["Blower cabinet", "temperature 2"]} />
      <RD x={1120} y={188} w={66} value="19.9" unit="°C" tag="DPT2-AEB0-BM2-TT1" name="Blower cabinet temp 2" group="MBBR" />
      <Eq title="Blower 2 MBBR" onClick={open("DPT2-AEB0-BL1")}>
      ...
      <Tag2 x={1153} y={330} tag="DPT2-AEB0-BM2" desc={["Blower 2 MBBR"]} />

      <Eq title="Bioreactor (MBBR)" onClick={open("DPT2-AEB0-BL1")}><Bioreactor x={930} y={410} w={340} h={180} /></Eq>
      <RD x={1017} y={448} w={66} value="261" unit="cm" tag="DPT2-AEB0-LT1" name="Level in bioreactor" group="MBBR" />
      <Tag2 x={1050} y={494} tag="DPT2-AEB0-LT1" desc={["Level in bioreactor"]} />

      {/* ───── CO₂ STRIPPER ───── */}
      <Tag2 x={1310} y={210} tag="DPT2-STR0-AV1" desc={["CO₂-fan 1"]} />
      <RD x={1277} y={248} value="50" unit="Hz" tag="DPT2-STR0-AV1" name="CO₂-fan 1 speed" group="CO₂ Stripper" />
      <Eq title="CO₂-fan 1" onClick={open("DPT2-STR0-FAN")}><SymFan cx={1310} cy={296} running /></Eq>
      <ModeChip x={1332} y={288} mode="A" />
      <SymTrend cx={1280} cy={296} tag="DPT2-STR0-AV1" name="CO₂-fan 1" group="CO₂ Stripper" running />

      <Tag2 x={1430} y={210} tag="DPT2-STR0-AV2" desc={["CO₂-fan 2"]} />
      <RD x={1397} y={248} value="50" unit="Hz" tag="DPT2-STR0-AV2" name="CO₂-fan 2 speed" group="CO₂ Stripper" />
      <Eq title="CO₂-fan 2" onClick={open("DPT2-STR0-FAN")}><SymFan cx={1430} cy={296} running /></Eq>
      <ModeChip x={1452} y={288} mode="A" />
      <SymTrend cx={1400} cy={296} tag="DPT2-STR0-AV2" name="CO₂-fan 2" group="CO₂ Stripper" running />

      <Eq title="CO₂ stripper" onClick={open("DPT2-STR0-FAN")}><StripperColumn x={1300} y={430} w={160} h={110} /></Eq>
      <RD x={1300} y={356} w={86} value="−24.6" unit="mbar" tag="DPT2-STR1-PT1" name="Vacuum in CO₂ stripping" group="CO₂ Stripper" />
      <Tag2 x={1380} y={400} tag="DPT2-STR1-PT1" desc={["Vacuum in CO₂ stripping"]} />

      {/* ───── PUMP SUMP ───── */}
      <RD x={1500} y={255} w={72} value="2.4" unit="mVs" tag="DPT2-SMP-PT1" name="Fish tank pressure" group="Pump Sump" />
      <Tag2 x={1536} y={232} tag="DPT2-SMP-PT1" desc={["Fish tank pressure"]} />
      <RD x={1640} y={255} w={66} value="11.6" unit="°C" tag="DPT2-SMP0-TT1" name="Pump sump temperature" group="Pump Sump" />
      <Tag2 x={1673} y={232} tag="DPT2-SMP0-TT1" desc={["Pump sump temperature"]} />

      <Eq title="Pump sump" onClick={open("DPT2-SMP0")}><SumpBasin x={1500} y={470} w={250} h={120} /></Eq>

      <RD x={1522} y={498} value="0" unit="Hz" tag="DPT2-SMP0-PU1" name="Lift pump 1 speed" group="Pump Sump" />
      <Eq title="Lift pump 1" onClick={d2Eq("DPT2-SMP0-PU1", "Lift pump 1", "pump", { primary: { l: "Speed", v: "0", u: "Hz" }, status: "low", readouts: [{ l: "Speed", v: "0", u: "Hz", tag: "DPT2-SMP0-PU1" }] })}><SymPump cx={1555} cy={548} running={false} /></Eq>
      <ModeChip x={1577} y={540} mode="A" />
      <Tag2 x={1555} y={588} tag="DPT2-SMP0-PU1" desc={["Lift pump 1"]} />

      <RD x={1637} y={498} value="36" unit="Hz" tag="DPT2-SMP0-PU2" name="Lift pump 2 speed" group="Pump Sump" />
      <Eq title="Lift pump 2" onClick={d2Eq("DPT2-SMP0-PU2", "Lift pump 2", "pump", { primary: { l: "Speed", v: "36", u: "Hz" }, readouts: [{ l: "Speed", v: "36", u: "Hz", tag: "DPT2-SMP0-PU2" }], setpoints: [{ key: "s", l: "Speed setpoint", v: 36, u: "Hz", step: 0.5, min: 0, max: 60 }], trend: { label: "Pump speed", base: 36, amp: 4, seed: 1.7, unit: "Hz", hi: 60 } })}><SymPump cx={1670} cy={548} running /></Eq>
      <ModeChip x={1692} y={540} mode="A" />
      <SymTrend cx={1640} cy={548} tag="DPT2-SMP0-PU2" name="Lift pump 2" group="Pump Sump" running />
      <Tag2 x={1670} y={588} tag="DPT2-SMP0-PU2" desc={["Lift pump 2"]} />
      ...
      {/* ───── outlets ───── */}
      <Flag x={1866} y={272} label="Fish tanks" dir="r" />
      <Flag x={1840} y={780} label="Oxygen" dir="l" />
      <Flag x={1866} y={947} label="Fish tanks" dir="r" />
